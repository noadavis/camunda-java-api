...
@Component
public class RejectEndEvent implements JavaDelegate {
    private static final Log log = LogFactory.getLog(RejectEndEvent.class);
    @Autowired
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        if (taskId > 0) {
            log.info(String.format("RejectEndEvent: task_id: %d", taskId));
            Optional<CustomTask> optionalTask = taskRepository.findById(taskId);
            if (optionalTask.isPresent()) {
                CustomTask t = optionalTask.get();
                t.setStatus(TaskStatus.REJECT);
                TaskHistory h = new TaskHistory();
                h.setUser(0);
                h.setUsername("system");
                h.setCustomTask(t);
                h.setDateCreated(new Date());
                h.setMessage("RejectEndEvent: status updated: " + t.getStatus().name());
                t.addHistory(h);
                taskRepository.save(t);
            } else log.error(String.format("RejectEndEvent delegate: CustomTask not found, taskId: %d", taskId));
        } else log.error(String.format("RejectEndEvent delegate: task_id: not found, ProcessInstanceId: %s", processInstanceId));
    }
}